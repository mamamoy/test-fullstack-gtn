...
const filteredReports = computed(() => {
    if (!searchTable.value) {
        ...
        return props.data.customers.filter(customer => customer.customer_name.toLowerCase().includes(query) || customer.telephone_number.includes(query) || customer.address.includes(query))
    }
})
// end
...
const openAddCustomerModal = () => {
    ...
    editCustomerMode.value = false
}
// end

// reset form customer data
...
const addCustomer = async () => {
    ...
    formCustomer.append('customer_name', customer_name.value)
    formCustomer.append('telephone_number', telephone_number.value)
    formCustomer.append('address', address.value)
    try {
        await router.post('customer/store', formCustomer, {
            onSuccess: page => {
                Swal.fire({
                    toast: true,
                    icon: 'success',
        ...
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Customer has not been added, try again!'
        })
...
const handleCustomerClose = (done) => {
    resetFormCustomer()
    done()
}
// end
...
const openEditModal = (customer) => {
    ...
    telephone_number.value = customer.telephone_number
...
const updateCustomer = async () => {
    ...
    formData.append('telephone_number', telephone_number.value)
    formData.append('address', address.value)
    formData.append('_method', 'PUT')

    try {
        await router.post('customer/update/' + id.value, formData, {
            onSuccess: (page) => {
                dialogCustomerVisible.value = false
                resetFormCustomer()
                Swal.fire({
...
// delete selected customer with sales reports relation with
const deleteCustomer = (customer) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                router.delete('customer/destroy/' + customer.id, {
                    onSuccess: (page) => {
                        Swal.fire({
                            toast: true,
                            icon: 'success',
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <h3 class="font-medium text-gray-900">List of Customers</h3>
                    ...
                    <!-- Dialog for Form Add/Edit Customers -->
                    <el-dialog v-model="dialogCustomerVisible"
                        :title="editCustomerMode ? 'Edit customer' : 'Add new customer'" width="500"
                        :before-close:="handleCustomerClose">


                        <form @submit.prevent="editCustomerMode ? updateCustomer() : addCustomer()" class="max-w-md mx-auto">
                    ...
                    <div
                        ...
                        <button @click="openAddCustomerModal" type="button"
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Telephone Number</th>
                        ...
                        <tbody>
                            <tr v-for="(customer, index) in paginatedReports" :key="customer.id"
                                class="border-b dark:border-gray-700">
                                <td class="px-4 py-3">{{ (pagination.currentPage - 1) * pagination.perPage + index + 1 }}
                                </td>
                                <th scope="row"
                                    class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ customer.customer_name }}</th>
                                <td class="px-4 py-3">{{ customer.telephone_number }}</td>
