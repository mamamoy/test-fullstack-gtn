...
const filteredUsers = computed(() => {
    if (!searchTable.value) {
        ...
        return props.data.filter(sales => sales.customer.customer_name.toLowerCase().includes(query) || sales.user.name.includes(query) || sales.sales_report_id.includes(query));
    }
})
// end
...
const resetFormData = () => {
    ...
    idPicture.value = []
    dialogIdPictureURL.value = ''
    homePicture.value = []
    dialogHomePictureURL.value = ''
}
...
const openEditModal = (report) => {
    ...
    sales_name.value = report.user.name
    customer_name.value = report.customer.customer_name
    telephone_number.value = report.customer.telephone_number
    address.value = report.customer.address
    packet_name.value = report.selling_packets.packet_name
...
const updateSalesReport = async () => {
    ...
    try {
        await router.post('approved/' + id.value, formData, {
            onSuccess: (page) => {
...
const deleteSalesReport = (report) => {
    Swal.fire({
        title: 'Are you sure ?',
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        <h3 class="font-medium text-gray-900">List of Sales Applied</h3>
                    ...
                    <!-- Dialog for Approving Sales Reports -->
                    <el-dialog v-model="dialogVisible" width="500" title="Application Form"
                        :before-close:="handleClose">


                        <form @submit.prevent="updateSalesReport()" class="max-w-md mx-auto">
                            ...
                            <div class="relative z-0 w-full mb-5 group">
                                <input type="text" v-model="sales_name" name="sales_name" id="sales_name"
                                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    placeholder=" " required readonly />
                            ...
                            <div class="relative z-0 w-full mb-5 group">
                                <input type="text" v-model="customer_name" name="customer_name" id="customer_name"
                                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    placeholder=" " required readonly />
                                <label for="customer_name"
                                    class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Customer
                                    Name</label>
                            </div>
                            <div class="relative z-0 w-full mb-5 group">
                                <input type="text" v-model="telephone_number" name="telephone_number" id="telephone_number"
                                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    placeholder=" " required readonly />
                                <label for="telephone_number"
                                    class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Telephone/phone
                                    number</label>
                            </div>
                            <div class="relative z-0 w-full mb-5 group">
                            ...
                            <div class="relative z-0 w-full mb-5 group">
                                <input type="text" v-model="packet_name" name="packet_name" id="packet_name"
                                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    placeholder=" " required readonly />
                                <label for="packet_name"
                                    class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Packet Name</label>
                            </div>
                            <div class="relative mb-5 z-0 w-full group">
                                <div class="mb-2">
                                    <h3 class="text-sm text-gray-900">Identification ID Images</h3>
                                </div>
                                <div class="flex flex-nowrap mb-8">
                                    <div v-for="image in id_images" :key="image.id" class="relative w-32 h-32">
                                        <div class="demo-image__preview">
                                            <el-image style="width: 100px; height: 100px" :src="`/${image.identification_id_image}`" :zoom-rate="1.2"
                                                :max-scale="7" :min-scale="0.2" :preview-src-list="[`/${image.identification_id_image}`]" :initial-index="4"
                                                fit="cover" />
                                        </div>
                            ...
                            <div class="relative mb-5 z-0 w-full group">
                                <div class="mb-2">
                                    <h3 class="text-sm text-gray-900">Home Images</h3>
                                </div>
                                <div class="flex flex-nowrap mb-8">
                                    <div v-for="image in home_images" :key="image.id" class="relative w-32 h-32">
                                        <div class="demo-image__preview">
                                            <el-image style="width: 100px; height: 100px" :src="`/${image.home_image}`" :zoom-rate="1.2"
                                                :max-scale="7" :min-scale="0.2" :preview-src-list="[`/${image.home_image}`]" :initial-index="4"
                                                fit="cover" />
                                        </div>
                            ...
                            <div class="text-end">
                                <button type="submit"
                                    class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Approve</button>
                            </div>
                        </form>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="(report, index) in paginatedUsers" :key="report.id"
                                ...
                                <td class="px-4 py-3">{{ report.customer.customer_name }}</td>
                                <td v-if="report.isApproved !== 0" class="px-4 py-3">Approved</td>
                                <td v-else class="px-4 py-3">Pending</td>

                                <td class="px-4 py-3 flex items-center justify-end">
                                    <div class="flex items-center gap-3">
                                        
                                        <button v-if="report.isApproved !== 1" type="button" @click="openEditModal(report)"
                                        ...
                                        <button type="button" @click="deleteSalesReport(report)"
